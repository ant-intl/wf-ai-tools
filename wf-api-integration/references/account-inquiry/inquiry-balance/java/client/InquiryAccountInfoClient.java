...
public class InquiryAccountInfoClient {
     ...
     * @throws WfException 调用失败时抛出
     */
    public InquirySubuserResponse inquirySubuser(InquirySubuserRequest request) {
        validateSubuserRequest(request);

        String requestBody = buildSubuserRequestBody(request);
        String url = config.getBaseUrl() + PATH_SUBUSER;

        LOGGER.info("InquiryAccountInfoClient invoking inquirySubuser, url=" + url);

        String responseBody = httpClientUtil.sendPostRequest(url, PATH_SUBUSER, requestBody);

        return parseSubuserResponse(responseBody);
    }
     ...
     * @throws WfException 校验失败时抛出
     */
    private void validateSubuserRequest(InquirySubuserRequest request) {
        if (request == null) {
            throw new WfException(WfErrorCode.PARAM_ILLEGAL, "InquirySubuserRequest must not be null");
        }
     ...
     * @return JSON 字符串
     */
    private String buildSubuserRequestBody(InquirySubuserRequest request) {
        JSONObject body = new JSONObject();
     ...
     * @throws WfException 响应格式非法或业务失败时抛出
     */
    private InquirySubuserResponse parseSubuserResponse(String responseBody) {
        InquirySubuserResponse response;
        try {
            response = JSON.parseObject(responseBody, InquirySubuserResponse.class);
        } catch (Exception e) {
            LOGGER.error("InquiryAccountInfoClient failed to parse inquirySubuser response, body=" + responseBody, e);
            throw new WfException(WfErrorCode.INVALID_RESPONSE_FORMAT,
                "Failed to parse inquirySubuser response: " + e.getMessage(), e);
        }

        Result result = response.getResult();
        if (result == null) {
            throw new WfException(WfErrorCode.INVALID_RESPONSE_FORMAT,
                "inquirySubuser response result is null");
        }

        String resultStatus = result.getResultStatus();
        if (RESULT_STATUS_SUCCESS.equals(resultStatus)) {
            LOGGER.info("InquiryAccountInfoClient inquirySubuser success, totalCount=" + response.getTotalCount()
                + ", currentPageNumber=" + response.getCurrentPageNumber());
            return response;
        } else if (RESULT_STATUS_FAIL.equals(resultStatus)) {
            WfErrorCode errorCode = WfErrorCode.fromCode(result.getResultCode());
            LOGGER.warn("InquiryAccountInfoClient inquirySubuser failed, resultCode=" + result.getResultCode()
                + ", resultMessage=" + result.getResultMessage());
            throw new WfException(errorCode, result.getResultMessage());
        } else {
            // resultStatus=U 或未知状态，交由调用方重试
